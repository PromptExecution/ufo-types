    ...
    fn satisfies(&self, constraint: &ResourceRequirements) -> crate::satisfies::SatisfiesResult {
        if constraint.fits_on(self) {
            ...
            let detail = format!(
                ...
                self.gpu_count,
                self.cpu_cores,
                constraint.min_ram_gb,
                constraint.min_vram_gb,
                constraint.requires_gpu,
                constraint.cpu_cores
            );
...
// 🤓 clippy::large_enum_variant wants `Spec` boxed, but that changes the
//    public API shape (every `StageEntry::Spec(spec)` match site) beyond
//    what this task's brief authorizes — suppressing keeps the type
//    identical to b00t-cli's original.
