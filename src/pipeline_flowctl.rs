        ...
        match &self.strategy {
            ...
            FlowStrategy::Throttled { .. } => {
                ...
                self.throttle_window_bytes =
                    self.throttle_window_bytes.saturating_add(bytes as u64);
...
mod tests {
    ...
    fn cpu_stage_gets_unbounded_strategy() {
        ...
        assert_eq!(
            strategy,
            FlowStrategy::Unbounded,
            "CPU stage should get Unbounded"
        );
    }
    ...
    fn high_memory_stage_gets_buffered_strategy() {
        ...
        match strategy {
            ...
            _ => panic!(
                "High-memory stage should get Buffered strategy, got {:?}",
                strategy
            ),
        }
    }
    ...
    fn wait_backpressure_returns_duration() {
        ...
        assert_eq!(
            unbounded.wait_backpressure(),
            Duration::ZERO,
            "Unbounded should have zero wait"
        );
